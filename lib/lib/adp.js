...
exports.getWorkers = exports.auth = void 0;
...
const auth = data => {
  ...
  const httpsAgent = new _https.default.Agent(certs);
  // Initializes Axios client with Agent and certs.
  const adp = _axios.default.create({
    httpsAgent
  });
  const options = {
    headers: {
  ...
  return adp.post('https://accounts.adp.com/auth/oauth/v2/token?grant_type=client_credentials', null, options).then(res => res.data);
};
exports.auth = auth;
